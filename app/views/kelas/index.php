...
/**
 * File View untuk menampilkan daftar kelas.
 * File ini menggunakan ekstensi Kartk GridView untuk menampilkan data dalam bentuk tabel
 * dan ExportMenu untuk fungsionalitas ekspor data, dengan styling modern.
 *
 ...
 * @var app\models\search\KelasSearch $searchModel - Model pencarian untuk filter tabel.
 */
...
// === JAVASCRIPT TAMBAHAN ===
// Mendaftarkan JavaScript untuk fungsionalitas tambahan seperti tooltip dan toggle search form
$this->registerJs("
    ...
    // JavaScript untuk mengaktifkan toggle search form
...
$gridColumns = [
    ...
    [
        ...
        'value' => function ($model) {
            // Menggunakan badge dengan styling gradien untuk menampilkan kode kelas.
            return '<span class="badge badge-primary" style="font-size: 0.9rem; padding: 6px 12px; background: linear-gradient(45deg, #4db6ac, #26a69a); border-radius: 20px; color: white;">'
                . Html::encode($model->kode_kelas) . '</span>';
        },
    ],
    [
        ...
        'contentOptions' => ['class' => 'text-center'],
        'format' => 'raw',
        'value' => fn($model) => '<strong style="color: #263238; font-size: 1.05rem;">' . ($model->kodeJenjang->kode_jenjang ?? '-') . '</strong>',
        'filterType' => GridView::FILTER_SELECT2,
        'filter' => \yii\helpers\ArrayHelper::map(\app\models\base\Jenjang::find()->asArray()->all(), 'kode_jenjang', 'kode_jenjang'),
        'filterWidgetOptions' => ['pluginOptions' => ['allowClear' => true]],
        'filterInputOptions' => ['placeholder' => 'Pilih Jenjang'],
    ],
    [
        ...
        'contentOptions' => ['class' => 'text-center'],
        'format' => 'raw',
        'value' => fn($model) => '<strong style="color: #263238; font-size: 1.05rem;">' . ($model->kodeJurusan->kode_jurusan ?? '-') . '</strong>',
        'filterType' => GridView::FILTER_SELECT2,
        'filter' => \yii\helpers\ArrayHelper::map(\app\models\base\Jurusan::find()->asArray()->all(), 'kode_jurusan', 'kode_jurusan'),
        'filterWidgetOptions' => ['pluginOptions' => ['allowClear' => true]],
        'filterInputOptions' => ['placeholder' => 'Pilih Jurusan'],
    ],
    [
        'attribute' => 'nama',
        'label' => '📖 Nama Kelas',
        ...
        'format' => 'raw',
        'value' => function ($model) {
            // Memberikan styling tebal pada teks nama kelas.
            return '<strong style="color: #263238; font-size: 1.05rem;">' . Html::encode($model->nama) . '</strong>';
        },
    ],
];
?>
...
<div class="kelas-index container">
    ...
    <h1><i class="fas fa-school"></i> <?= Html::encode($this->title) ?></h1>

    <!-- Kontainer untuk tombol aksi dan export -->
    <div class="button-container">
        <!-- Tombol untuk menambah kelas baru -->
        <?= Html::a(
            '<i class="fas fa-plus-circle"></i> Tambah Kelas Baru',
            ['create'],
            [
                'class' => 'btn btn-tambah',
                'title' => 'Klik untuk menambah kelas baru',
                'data-toggle' => 'tooltip',
                'data-placement' => 'top'
            ]
        ) ?>
    ...
    <?= GridView::widget([
                ...
                'buttons' => [
                    ...
                    'view' => function ($url, $model) {
                        return Html::a(
                            ...
                            ['view', 'id' => $model['kode_kelas']],
                            [
                                'title' => 'Lihat detail kelas: ' . $model['nama'],
                                'data-toggle' => 'tooltip',
                                'data-placement' => 'top'
                            ]
                        );
                    ...
                    'update' => function ($url, $model) {
                        return Html::a(
                            ...
                            ['update', 'id' => $model['kode_kelas']],
                            [
                                'title' => 'Ubah kelas: ' . $model['nama'],
                                'data-toggle' => 'tooltip',
                                'data-placement' => 'top'
                            ]
                        );
                    ...
                    'delete' => function ($url, $model) {
                        return Html::a(
                            ...
                            ['delete', 'id' => $model['kode_kelas']],
                            [
                                'title' => 'Hapus kelas: ' . $model['nama'],
                                'data-toggle' => 'tooltip',
                                'data-placement' => 'top',
                                'data' => [
                                    // Konfirmasi saat tombol hapus ditekan.
                                    'confirm' => 'Apakah Anda yakin ingin menghapus kelas "' . $model['nama'] . '"? Tindakan ini tidak dapat dibatalkan.',
                                    'method' => 'post',
                                ],
                            ]
                        );
        ...
        'summary' => '<div class="text-center mt-3 mb-3">
                        <strong>📊 Menampilkan <span style="color: #00897b;">{begin}-{end}</span> dari total <span style="color: #00bfa5;">{totalCount}</span> data kelas</strong>
                    </div>',
